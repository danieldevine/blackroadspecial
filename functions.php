<?php

include_once('inc/functions/BRS_register-menus.php');
include_once('inc/functions/BRS_add-buttons.php');
include_once('inc/functions/BRS_enqueue-styles-scripts.php');
include_once('inc/functions/BRS_custom-posts.php');
include_once('inc/functions/BRS_shortcodes.php');
include_once('inc/functions/BRS_thumbnail-sizes.php');
include_once('inc/functions/BRS_search-form.php');
include_once('inc/functions/BRS_contact-form.php');
include_once('inc/functions/BRS_head-cleaner.php');
include_once('inc/functions/BRS_post-link-classes.php');
